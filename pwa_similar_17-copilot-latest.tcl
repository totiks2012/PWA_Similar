...
# Функция для скачивания иконки через Bash-скрипт (асинхронно)
proc download_icon_async {url app_name callback} {
    set script_path [file join [pwd] "download_icon.sh"]
    if {[catch {
        set result [exec $script_path $url $app_name 2>@1]
    } error]} {
        puts "Error executing Bash script: $error"
        after 0 [list {*}$callback ""]
        return
    }
    ...
    after 0 [list {*}$callback $icon_path]
}
...
button .controls.clear_search -text "✕" -command {
    puts "Clear search button clicked"
...
# Кнопки управления
button .controls.add_btn -text "+" -command {
...
button .controls.del_btn -text "-" -command {
    puts "Delete button clicked"
    delete_url
} -font [list "TkDefaultFont" $control_font_size] -foreground $font_color -background $bg_color -cursor arrow
pack .controls.del_btn -side left -padx 5

button .controls.add_tab_btn -text "Tab+" -command {
    puts "Add tab button clicked"
    show_new_tab_window
} -font [list "TkDefaultFont" $control_font_size] -foreground $font_color -background $bg_color -cursor arrow
pack .controls.add_tab_btn -side left -padx 5

button .controls.up_btn -text "↑" -command {
    puts "Move up button clicked"
    move_up
} -font [list "TkDefaultFont" $control_font_size] -foreground $font_color -background $bg_color -cursor arrow
pack .controls.up_btn -side left -padx 5

button .controls.down_btn -text "↓" -command {
    puts "Move down button clicked"
    move_down
} -font [list "TkDefaultFont" $control_font_size] -foreground $font_color -background $bg_color -cursor arrow
pack .controls.down_btn -side left -padx 5

button .controls.io_btn -text "⇄" -command {
    puts "IO button clicked"
    show_io_window
} -font [list "TkDefaultFont" $control_font_size] -foreground $font_color -background $bg_color -cursor arrow
...
proc show_new_tab_window {} {
    ...
    bind .new_tab_win.name_entry <Return> {
        puts "Enter key pressed in new tab entry"
        set tab_name [.new_tab_win.name_entry.get]
        if {$tab_name ne "" && $tab_name ni $::tab_list} {
            add_tab $tab_name
            send_notification "PWA Similar Manager" "Создана новая вкладка: $tab_name"
...
proc lremove {list item} {
    set idx [lindex lsearch -exact $list $item]
    if {$idx >= 0} {
...
# Привязка кликов мыши к вкладкам
bind .tabs <Button-1> { 
...
proc show_add_window {} {
    ...
    proc create_application {url app_name tab_name create_desktop parent} {
        ...
        if {$url ne ""} {
            ...
            if {$create_desktop} {
                ...
                download_icon_async $url $app_name [list create_pwa_async $url $app_name $tab_name $parent]
            } else {
    ...
    proc create_pwa_async {url app_name tab_name parent icon_path} {
        set icon_dir [ensure_icon_directory]
        set default_icon [file join $icon_dir "web_apps.png"]
        set final_icon_path ""
        if {$icon_path ne "" && [is_valid_icon $icon_path]} {
            set final_icon_path $icon_path
        } elseif {[file exists $default_icon] && [is_valid_icon $default_icon]} {
            set final_icon_path $default_icon
        } else {
            set final_icon_path "application-x-executable"
        }
    ...
    button .add_win.btn -text "Создать" -command {
        puts "Create PWA button clicked"
    ...
    button .add_win.paste_btn -text "Вставить из буфера" -command {
        puts "Paste button clicked"
        paste_from_clipboard
    } -font [list "TkDefaultFont" $control_font_size] -foreground $font_color -background $bg_color -cursor arrow
    place .add_win.paste_btn -x 150 -y 240

    bind .add_win.url_entry <Return> {
        puts "Enter key pressed in URL entry"
        create_application [.add_win.url_entry get] [.add_win.name_entry get] [.add_win.tab_entry get] $create_desktop_var .add_win
    }
    bind .add_win.name_entry <Return> {
        puts "Enter key pressed in name entry"
        create_application [.add_win.url_entry get] [.add_win.name_entry get] [.add_win.tab_entry get] $create_desktop_var .add_win
    }
    bind .add_win.tab_entry <Return> {
        puts "Enter key pressed in tab entry"
        create_application [.add_win.url_entry get] [.add_win.name_entry get] [.add_win.tab_entry get] $create_desktop_var .add_win
    }
    bind .add_win.url_entry <Control-v> {
        puts "Ctrl+V pressed in URL entry"
        paste_from_clipboard
    }
...
# Всплывающие подсказки (временно отключены для кнопок, чтобы проверить влияние)
